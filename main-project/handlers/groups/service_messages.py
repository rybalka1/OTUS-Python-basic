...
async def add_new_chat(*args):
    pool: Connection = db
    chat_find: Record = await pool.fetchval(
        """SELECT chat_id FROM bot_chat WHERE chat_id= $1""",
        args[0],
    )
    if not chat_find:
        ...
        if not item.is_bot:
            ...
            if member:
                ...
                await message.answer(
                    f'{member.get("last_name")} {member.get("first_name")} с возвращением!'
                )
                if not member["capcha"]:
                    ...
                    await message.chat.restrict(
                        item.id,
                        permissions=(
                            await message.bot.get_chat(message.chat.id)
                        ).permissions,
                    ...
                    await message.answer(
                        text=txt, reply_markup=await antibot_cb_create(item.id, rd)
                    )
            else:
                arg = (
                    item.id,
...
async def bot_left_all_chats_member(message: types.Message):
    ...
    if message.left_chat_member.id == message.from_user.id:
        ...
        await message.answer(
            f"{message.left_chat_member.full_name} кикнут из чата "
...
async def bot_new_chat_title(message: types.Message):
    ...
    if not await add_new_chat(
        ...
        await pool.fetchval(
            """UPDATE bot_chat SET chat_id=$2, chat_type=$3 WHERE chat_id=$1""", *arg
        )
        arg = message.chat.id, message.migrate_to_chat_id
        await pool.fetchval(
            """UPDATE chat_users SET chat_id=$2 WHERE chat_id=$1""", *arg
        )
