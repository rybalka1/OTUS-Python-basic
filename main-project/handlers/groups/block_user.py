...
@rate_limit(1, 'ro')
@dp.message_handler(chat_type=ChatType.GROUP, is_chat_admin=True, commands='ro', commands_prefix='!/',
                    is_reply=True)
...
@rate_limit(1, 'ro')
@dp.message_handler(commands_prefix='!/', chat_type=ChatType.SUPERGROUP, is_chat_admin=True, commands='ro',
                    is_reply=True, )
async def bot_ro(message: types.Message):
    ...
    if status_user not in ['administrator', 'creator']:
        ...
        min_p = ' min'
        try:
            await message.chat.restrict(who_ban,
...
@rate_limit(1, 'unro')
@dp.message_handler(chat_type=ChatType.GROUP, is_chat_admin=True, commands='unro', commands_prefix='!/',
                    is_reply=True)
...
@rate_limit(1, 'unro')
@dp.message_handler(commands_prefix='!/', chat_type=ChatType.SUPERGROUP, is_chat_admin=True, commands='unro',
                    is_reply=True)
async def bot_unro(message: types.Message):
    ...
    timestamp_utc = calendar.timegm(time_tuple)
    who_ban = message.reply_to_message.from_user.id
    await message.chat.restrict(message.reply_to_message.from_user.id, (await message.bot.get_chat(message.chat.id)).permissions, until_date=timestamp_utc)
...
@rate_limit(1, 'report')
@dp.message_handler(chat_type=[ChatType.GROUP, types.ChatType.SUPERGROUP], commands='report', is_reply=True,
                    commands_prefix='!/')
async def bot_report(message: types.Message):
    pool: Connection = db
    get_chat_admins: Record = await pool.fetch('''SELECT * 
                                                  FROM chat_users 
                                                  WHERE status='administrator' OR status = 'creator';''')
    for admin in get_chat_admins:
        msg = (f"Пользователь <a href='tg://user?id={message.from_user.id}'>"
...
@dp.message_handler(commands_prefix='!/', chat_type=[ChatType.SUPERGROUP, ChatType.GROUP], is_chat_admin=True,
                    commands='warn', is_reply=True)
async def bot_warn(message: types.Message):
    pool: Connection = db
    warn = await pool.fetchval('''UPDATE chat_users 
                                  SET karma=(karma+1) 
                                  WHERE user_id=$1 AND chat_id=$2 RETURNING karma''',
                               message.reply_to_message.from_user.id, message.chat.id)
    if warn % 3 != 0:
        await message.reply(f'У пользователя {message.reply_to_message.from_user.full_name} {warn % 3}/3 предупреждений')
