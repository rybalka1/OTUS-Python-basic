        ...
        try:
            ...
            if message.reply_to_message.content_type == "text":
                ...
                find_tg: Record = await pool.fetchrow(
                    """SELECT tg_name FROM triggers WHERE tg_name = $1 and chat_id=$2""",
                    *arg,
                )
...
async def bot_trigger_del(message: types.Message):
    if message.text != "-trigger":
        try:
            ...
            name_trigger = message.text.split("-trigger", maxsplit=1)[1].strip()
            arg = name_trigger, message.chat.id
            find_tg = await pool.fetchval(
                """SELECT id FROM triggers WHERE tg_name = $1 and chat_id=$2""",
                *arg,
            )
            if find_tg:
                if await pool.fetchrow(
                    """delete from triggers WHERE id=$1 RETURNING TRUE""",
                    find_tg,
                ):
                    await message.answer(f"триггер <b>{name_trigger}</b> был удалён")
                else:
                    await message.answer(
                        f"Произошла ошибка, не смог удалить триггер {name_trigger}"
                    )
            else:
...
async def bot_trigger_list(message: types.Message):
    try:
        pool: Connection = db
        find_tg: Record = await pool.fetch(
            """SELECT tg_name, tg_type FROM triggers WHERE chat_id=$1""",
            message.chat.id,
        )
        list_tg = []
        if find_tg:
...
async def bot_trigger_activate(message: types.Message):
    ...
    trig: Record = await pool.fetchrow(
        """SELECT * from triggers where chat_id= $1 and tg_name = $2""", *arg
    )
