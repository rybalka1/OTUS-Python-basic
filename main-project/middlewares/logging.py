...
class LoggingMiddleware(BaseMiddleware):
    ...
    async def on_post_process_my_chat_member(
        self, my_chat_member_update, results, data
    ):
        self.logger.debug(
            f"{HANDLED_STR[bool(len(results))]} my_chat_member "
            f"for user [ID:{my_chat_member_update.from_user.id}]"
        )
    ...
    async def on_post_process_chat_member(self, chat_member_update, results, data):
        self.logger.debug(
            f"{HANDLED_STR[bool(len(results))]} chat_member "
            f"for user [ID:{chat_member_update.from_user.id}]"
        )
...
class LoggingFilter(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord):
        ...
        if update:
            for key, value in self.make_prefix(
                self.prefix, self.process_update(update)
            ):
    ...
    def process_message(self, message: types.Message):
        ...
        if message.reply_to_message:
            yield from self.make_prefix(
                "reply_to", self.process_message(message.reply_to_message)
            )
        if message.forward_from:
            yield from self.make_prefix(
                "forward_from", self.process_user(message.forward_from)
            )
    ...
    def process_chosen_inline_result(
        self, chosen_inline_result: types.ChosenInlineResult
    ):
