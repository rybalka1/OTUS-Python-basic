    ...
    for item in message.new_chat_members:
        if not item.is_bot:
            ...
            if member:
                ...
                # TODO: ограничить права и добавить лёгкий анстиспам инлайн режим
                await message.answer(f'{message.from_user.full_name} ну, допустим, привет!')
...
async def bot_left_all_chats_member(message: types.Message):
    ...
    if message.left_chat_member.id == message.from_user.id:
        ...
        await message.answer(f'{await message.left_chat_member.full_name} кикнут из чата '
...
async def bot_new_chat_title(message: types.Message):
    ...
    if not await add_new_chat(message.chat.id, message.chat.type, True, message.chat.title,
        ...
        await pool.fetchval('''UPDATE statistics SET chat_id=$2 WHERE chat_id=$1''', *arg)
